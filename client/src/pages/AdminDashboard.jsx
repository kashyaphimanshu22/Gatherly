...
const AdminDashboard = () => {
    ...
    return (
        <div className="max-w-7xl mx-auto">
            <div className="bg-gradient-to-r from-indigo-700 via-blue-700 to-cyan-600 text-white rounded-3xl p-10 mb-10 shadow-2xl flex flex-col lg:flex-row justify-between items-center gap-8 overflow-hidden relative">
                <div>
                    <h1 className="text-4xl lg:text-5xl font-black mb-3">Admin Dashboard</h1>
                    <p className="text-blue-100 text-lg">Manage events, approve bookings and monitor platform activity in one place.</p>
                </div>
                <button
                    onClick={() => setShowEventForm(!showEventForm)}
                    className="bg-white text-indigo-700 font-bold px-8 py-4 rounded-2xl shadow-xl hover:scale-105 hover:bg-gray-100 transition duration-300"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10">
                <div className="bg-gradient-to-br from-green-500 to-emerald-600 text-white p-7 rounded-3xl shadow-xl hover:scale-105 transition duration-300 flex justify-between items-center">
                    <div>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
                    <div>
                        <p className="text-green-100 uppercase text-sm font-bold tracking-widest"></p>
                        <h3 className="text-4xl font-black mt-2">{new Set(bookings.filter(b => b.paymentStatus === 'paid' && b.status === 'confirmed').map(b => b.userId?._id)).size}</h3>
                    </div>
                    <div className="w-12 h-12 bg-blue-100 text-blue-500 rounded-full flex items-center justify-center text-xl font-bold">👤</div>
                </div>
                <div className="bg-gradient-to-br from-blue-500 to-indigo-600 text-white p-7 rounded-3xl shadow-xl hover:scale-105 transition duration-300 flex justify-between items-center">
                    <div>
                        <p className="text-blue-100 uppercase text-sm font-bold tracking-widest"></p>
                        <h3 className="text-4xl font-black mt-2">{bookings.filter(b => b.status === 'pending').length}</h3>
                    </div>
                    <div className="w-16 h-16 rounded-2xl bg-white/20 flex items-center justify-center text-3xl">👥</div>
                </div>
            </div>
            ...
            {showEventForm && (
                <div className="bg-gradient-to-br from-orange-400 to-red-500 text-white p-7 rounded-3xl shadow-xl hover:scale-105 transition duration-300 flex justify-between items-center">                    <h2 className="text-2xl font-bold mb-6 text-gray-800">Create New Event</h2>
